...
class MotionPlanner:
    ...
    def cmd_callback(self, data):
        ...
        if cmd_type == "move_arc" or cmd_type == "move_line":
            ...
            rospy.Timer(rospy.Duration(1.0 / self.RATE), self.mode)
    ...
    def arc_move(self):
        ...
        try:
            ...
            self.norm_d = np.linalg.norm(distance_map_frame[:2])
            self.sqrt_d = np.sqrt(x_diff**2 + y_diff**2) # rho,

            path_done = np.sqrt(self.d_init**2 + self.alpha_init**2) - np.sqrt(d**2 + alpha**2)
            path_left = np.sqrt(d**2 + alpha**2)

            v = max(self.V_MIN, min(self.Kp * self.sqrt_d, self.V_MAX))
