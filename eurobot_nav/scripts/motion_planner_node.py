...
class MotionPlannerNode:
    def __init__(self):
        ...
        [-0.04221, -0.0751, 4.19], [-0.02999, 0.08194, 3.47], [0.06634, -0.00163, 0]])
    ...
    def proximity_callback(self, data):
        ...
        for i in range(self.sensor_coords.shape[0]):
            points = np.append(points, np.array([cvt_local2global(np.array[distances[i], 0, 0.], self.sensor_coords[i, :])]))
    ...
    def move(self):
        ...
        if self.current_state == "stop":
            ...
            #self.goal = cvt_global2local(self.path[0, :], self.coords)
            dist = np.linalg.norm(self.goal[:2])
            self.set_speed(np.zeros(3))
            #if dist < 1e-4:
            #    self.goal = self.coords
            #else:
            #    self.goal[:2] *= 0.1 / dist
            #    self.goal = cvt_local2global(self.goal, self.coords)
            #self.create_linear_path()
            #self.start_collision_point = self.coords
            #self.dist_to_start_collision_point = np.linalg.norm(self.coords[:2] - self.start_collision_point[:2])
...
if __name__ == "__main__":
    ...
    class MotionPlannerNode:
        def __init__(self):
            ...
            self.sensors_coords_vector = np.array(
                [[-1, 0], [np.sin(np.pi / 6), np.cos(np.pi / 6)], [np.sin(np.pi / 6), -np.cos(np.pi / 6)]])
            self.sensor_ind = 0
        ...
        def scan_callback(self, scan):
            ...
            # rospy.loginfo("NEAREST LANDMARK")
            # rospy.loginfo(np.linalg.norm(self.coords[:2] - landmarks, axis=1))
            self.obstacle_points = landmarks[np.argmin(np.linalg.norm(self.coords[:2] - landmarks, axis=1))]
            self.set_collision_point(self.obstacle_points)
            # else:
            #   self.obstacle_points = np.array([100, 100])
        ...
        def alpha_filter(cloud, min_sin_angle):
            ...
            x0, y0 = 0, 0
            x1, y1 = np.roll(x, 1), np.roll(y, 1)
            cos_angle = ((x - x0) * (x - x1) + (y - y0) * (y - y1)) / (
            np.sqrt((x - x0) * (x - x0) + (y - y0) * (y - y0))
            * np.sqrt((x - x1) * (x - x1) + (y - y1) * (y - y1)))
            sin_angle = np.sqrt(1 - cos_angle * cos_angle)
            index = sin_angle >= min_sin_angle
            return index
