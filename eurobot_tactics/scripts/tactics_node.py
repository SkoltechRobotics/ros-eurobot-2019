...
# from tactics_math import calculate_pucks_configuration
# from tactics_math import calculate_landings
...
class TacticsNode:
    def __init__(self):
        ...
        # publishers
        self.move_command_publisher = rospy.Publisher('command', String, queue_size=10)
        # self.stm_command_publisher = rospy.Publisher('stm/command', String, queue_size=1)
        ...
        rospy.Subscriber("/secondary_robot/pucks", MarkerArray, self.chaos_pucks_coords_callback, queue_size=1)
    ...
    def response_callback(self, data):
        ...
        data_split = data.data.split()
        cmd_id = data_split[0]
        cmd_status = data_split[1]

        if cmd_status == 'success':  # TODO change so that response is marked by cmd_id ''
