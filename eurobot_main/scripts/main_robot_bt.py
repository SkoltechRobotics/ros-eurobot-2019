...
class MainRobotBT(object):
    ...
    def change_side(self, side):
        ...
        self.russtr = RusStr(self.side_status)
        # self.safe = MadSafe(self.side_status)

        self.strategy = self.russtr
    ...
    def change_strategy(self, num):
        ...
        if num == 0:
            ...
            self.strategy = MadSafe(self.side_status)
        elif num == 2:
            print("CHANGE STRATEGY TO " + str(num))
...
class StrategyConfig(object):
    def __init__(self, side):  # pucks_slave
        ...
        self.HPAD = np.array(rospy.get_param(
            "horiz_pucks_approach_dist"))  # 0.127 meters, distance from robot to puck where robot will try to grab it
        ...
        self.goldenium_grabbed_flag = bt.BTVariable(False)
        ...
        self.first_puck_landing_near = np.array([self.first_puck_landing[0] - self.sign * 0.15,
        ...
        self.blunium_nose_start_push_pose = np.array([self.blunium[0] + self.sign * 0.135,  # 0.11
                                                      self.blunium[1] + self.robot_outer_radius - 0.025,
                                                      # 0.03 to close
                                                      0.56 - self.sign * 0.07])  # y/p  /0.63 or 0.56 both

        self.blunium_nose_end_push_pose = np.array([self.blunium[0] - self.sign * 0.22,  # 0.22
                                                    self.blunium_nose_start_push_pose[1] - 0.01,
                                                    # self.blunium[1] + 0.13
        ...
        self.goldenium_blind_move_pose_side = np.array([self.goldenium_blind_move_pose[0] - self.sign * 0.015,
                                                        self.goldenium_blind_move_pose[1],
                                                        self.goldenium_blind_move_pose[2]
                                                    ])
        ...
        self.scales_back_rotate = np.array([self.scales_goldenium_pos[0] + self.sign * 0.05,
                                            self.scales_goldenium_pos[1] - 0.25,
                                            -1.57 + self.sign * 1.01])

        self.unload_goldenium_on_blue = np.array([1.5 + self.sign * 1.3,  # y/p 2.7 / 0.3
                                                  0.9,
                                                  -1.57 + self.sign * 0.95])  # y/p -0.78 / -2.35
    ...
    def pucks_callback(self, data):  # parse_pucks
        ...
        try:
            ...
            if self.is_robot_started.get() is False:
                ...
                purple_chaos_pucks, yellow_chaos_pucks, purple_pucks_rgb, yellow_pucks_rgb = initial_parse_pucks(
            ...
            if self.is_robot_started.get() is True:
                ...
                print
                self.visible_pucks_on_field.get()
                print
                " "
    ...
    def update_lost_pucks(self):
        self.visible_pucks_on_field_sorted.set(np.delete(self.visible_pucks_on_field_sorted.get(), 0, axis=0))
        rospy.loginfo("visible_pucks_on_field_sorted: " + str(self.visible_pucks_on_field_sorted.get()))
    ...
    def calculate_pucks_configuration(self):
        ...
        if len(self.my_chaos_pucks.get()) >= 3:
            ...
            if not is_hull_safe_to_approach:
                self.my_chaos_pucks.set(
                    coords_sorted_by_angle)  # calc vert-angle, sort by angle, return vertices (sorted)
    ...
    def set_goldenium_grabbed_flag(self):
        self.goldenium_grabbed_flag.set(True)

    def is_goldenium_grabbed(self):
        if self.goldenium_grabbed_flag.get():
            return bt.Status.SUCCESS
        else:
            return bt.Status.FAILED
...
class RusStr(StrategyConfig):
    def __init__(self, side):  # pucks_slave
        super(RusStr, self).__init__(side)  # pucks_slave

        collect_red_cell_puck = bt.ParallelWithMemoryNode([
                                    bt.SequenceWithMemoryNode([
                                        bt_ros.MoveLineToPoint(self.first_puck_landing_near, "move_client"),
        ...
        collect_blue_cell_puck = bt.FallbackWithMemoryNode([
            bt.SequenceWithMemoryNode([
                bt_ros.StartCollectGroundCheck("manipulator_client"),
        ...
        try_push_blunium = bt.SequenceWithMemoryNode([
            bt_ros.SetSpeedSTM([-0.05, -0.1, 0], 1.4, "stm_client"),
            bt.ParallelWithMemoryNode([
        ...
        try_collect_goldenium = bt.SequenceWithMemoryNode([
            ...
            bt.ParallelNode([
                ...
                bt.Latch(bt_ros.MoveLineToPoint(self.goldenium_blind_move_pose_side, "move_client")),
                bt_ros.DelayNode(5)
            ], threshold=1),
            ...
            bt.FallbackWithMemoryNode([
                bt.SequenceWithMemoryNode([
                    ...
                    bt.ActionNode(self.set_goldenium_grabbed_flag),
                    bt.ActionNode(lambda: self.score_master.reward("GRAB_GOLDENIUM_BONUS"))
                ]),
                bt.SequenceWithMemoryNode([
                    bt_ros.MoveLineToPoint(self.goldenium_back_pose + np.array([0, 0.05, 0]), "move_client"),
                    bt_ros.StartCollectGoldenium("manipulator_client"),
                    bt.ParallelNode([
                        bt_ros.CheckBarometr("manipulator_client"),
        ...
        try_unload_goldenium = bt.FallbackWithMemoryNode([
                                    bt.SequenceWithMemoryNode([
                                        bt.ConditionNode(self.is_goldenium_grabbed),
                                        bt.SequenceNode([
                                            bt_ros.CheckBarometr("manipulator_client"),
                                            ...
                                            bt.Latch(
                                                bt.SequenceWithMemoryNode([
                                                    ...
                                                    bt_ros.MoveLineToPoint(self.scales_goldenium_pos + np.array([0, -0.07, 0]),
                                                                           "move_client")
                                                ])
                                            )
                                        ]),
                                        ...
                                        #bt_ros.MoveLineToPoint(self.scales_goldenium_pos + np.array([0, -0.04, 0]), "move_client"),
                                            ...
                                            bt_ros.SetToScales_ifReachedGoal(self.scales_goldenium_pos + np.array([0, -0.03, 0]), "manipulator_client", threshold=0.035),
                                            bt_ros.PublishScore_ifReachedGoal(self.scales_goldenium_pos + np.array([0, -0.04, 0]), self.score_master, "SCALES", threshold=0.04)
                                        ], threshold=3),

                                        #bt_ros.SetSpeedSTM([0.15, 0, 0], 1.4, "stm_client"),
        ...
        # try_take_two_lost = bt.SequenceWithMemoryNode([
        #
        ...
        #             bt_ros.StopPump("manipulator_client"),
        #             bt_ros.SetManipulatortoUp("manipulator_client"),
        #             bt_ros.MoveLineToPoint(self.scales_back_rotate, "move_client")
        #         ])
        #     ]),
        #
        #     bt.FallbackWithMemoryNode([
        ...
        final_search_unload2 = bt.SequenceWithMemoryNode([
            bt.ConditionNode(self.is_lost_puck_present),
            bt.ActionNode(self.sort_lost_wrt_robot),
            bt.ActionNode(lambda: self.calculate_next_landing(self.visible_pucks_on_field_sorted.get()[0])),
            # bt.ActionNode(self.calculate_next_prelanding),
        ...
        final_search_unload3 = bt.SequenceWithMemoryNode([
            bt.ConditionNode(self.is_lost_puck_present),
            bt.ActionNode(self.sort_lost_wrt_robot),
            bt.ActionNode(lambda: self.calculate_next_landing(self.visible_pucks_on_field_sorted.get()[0])),
            # bt.ActionNode(self.calculate_next_prelanding),
        ...
        final_search_unload4 = bt.SequenceWithMemoryNode([
            bt.ConditionNode(self.is_lost_puck_present),
            bt.ActionNode(self.sort_lost_wrt_robot),
            bt.ActionNode(lambda: self.calculate_next_landing(self.visible_pucks_on_field_sorted.get()[0])),
            # bt.ActionNode(self.calculate_next_prelanding),
        ...
        final_search_unload5 = bt.SequenceWithMemoryNode([
            bt.ConditionNode(self.is_lost_puck_present),
            bt.ActionNode(self.sort_lost_wrt_robot),
            bt.ActionNode(lambda: self.calculate_next_landing(self.visible_pucks_on_field_sorted.get()[0])),
            # bt.ActionNode(self.calculate_next_prelanding),
        ...
        self.tree = bt.SequenceWithMemoryNode([
            ...
            collect_blue_cell_puck,
            push_blunium_collision_delay,
            try_collect_goldenium,
            try_unload_goldenium,
            scales_back_rotate,
            search_lost_puck_unload_cell,
            final_search_unload,
            final_search_unload2,
            final_search_unload3,
            final_search_unload4,
            final_search_unload5
        ])


class MadSafe(StrategyConfig):
    def __init__(self, side):  # pucks_slave
        super(MadSafe, self).__init__(side)  # pucks_slave
                    ...
                    bt.SequenceNode([
                        ...
                        bt.Latch(
                            bt.SequenceWithMemoryNode([
                                ...
                                bt_ros.MoveLineToPoint(self.scales_goldenium_PREpos, "move_client"),
                                bt.ParallelWithMemoryNode([
                                    bt_ros.MoveLineToPoint(self.scales_goldenium_pos + np.array([0, -0.01, 0]),
                                                           "move_client"),
                                    bt_ros.SetToScales_ifReachedGoal(
                                        self.scales_goldenium_pos + np.array([0, -0.03, 0]), "manipulator_client",
                                        threshold=0.03),
                                    bt_ros.PublishScore_ifReachedGoal(
                                        self.scales_goldenium_pos + np.array([0, -0.03, 0]), self.score_master,
                                        "SCALES", threshold=0.03),
                                ], threshold=3)
                            ])
                        )
                    ]),
            ...
            bt.FallbackWithMemoryNode([
                bt.SequenceWithMemoryNode([
                    ...
                    bt.ParallelWithMemoryNode([
                        ...
                        bt.SequenceWithMemoryNode([
                            bt_ros.MoveLineToPoint(self.chaos_after_first, "move_client"),
                            # to prevent moving close to wall
